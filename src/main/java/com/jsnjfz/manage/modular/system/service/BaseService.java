...
public class BaseService<T> {
    ...
    public T selectByNum(Integer id){
        if (StringUtils.isEmpty(id)) {
            return null;
        } else {
            return baseDao.selectByNum(id);
        }
    }
...
//    }
     ...
     * @param title
     */
    public List<T> getTile(String title){
        if (StringUtils.isEmpty(title)){
            return null;
        } else {
            return baseDao.selectByTitle(title);
        }
    }
     ...
     * @param id
     */
    public Integer updateNum(Integer num,Integer id){
//        Integer coun = baseDao.updateByNum(num,id);
        return 1;
    }
    ...
    public int getPost(Map<String, Object> map){
        return baseDao.getPostId(map);
    }

    /**
     * 返回getCategoryId对应的数据
     */
    public List<T> getCategoryId(String id){
        return baseDao.getCategoryId(id);
    }
}
